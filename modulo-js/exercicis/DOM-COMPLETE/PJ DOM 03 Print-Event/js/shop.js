...
function applyPromotionsCart(id) {
    ...
    */
	...
	if (cart[id].quantity >= 3 && cart[id].id == 1) {
		...
		cart[id].subtotalWithDiscount =
			cart[id].quantity * products[cart[id].id].priceDiscount
		console.log(
			"ha comprat 3 o més ampolles d'oli. Preu total amb descompte:" +
				cart[id].subtotalWithDiscount
		)
		console.log('\n')
	}
	...
	if (cart[id].quantity >= 10 && cart[id].id == 3) {
		cart[id].subtotalWithDiscount =
			(cart[id].quantity * (cart[id].price * 2)) / 3
		console.log(
			'ha comprat 10 o més mescles de pastis. Preu total amb descompte:' +
				cart[id].subtotalWithDiscount
		)
		console.log('\n')
	}
...
function addCarrito(id) {
	...
	if (objIndex == -1) {
		...
		cart[objIndex].subtotal = parseFloat(
			(cart[objIndex].subtotal + cart[objIndex].price).toFixed(2)
		)
